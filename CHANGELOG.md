...
+ [修复] 启动恢复会话时缺少 rollout 的中间提示改为 debug 日志，最终恢复失败仍明确报错。

+ [修复] 二合一开发启动脚本收到终端关闭信号时同步停止画布和 Agent，避免 Agent 残留占用端口。

+ [优化] 本地 Agent 普通模式仅显示启动信息、警告和简短错误，详细运行日志改由 --debug 开启。

+ [修复] 本地 Agent 新建空对话在首轮开始前只读取会话摘要，避免 Codex 返回 list_turns 不支持并反复报错。

+ [修复] Agent 端口被占用时明确退出，不再误报启动成功或继续争用历史会话。

+ [新增] 支持在项目根目录用一条命令启动画布网页和本地 Agent，并统一停止，无需新增依赖。

+ [调整] 本地 Canvas Agent 复用已安装的 Codex CLI，不再下载 Codex 副本，并支持指定可执行文件路径。
